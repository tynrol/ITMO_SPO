...
int parse_insert(struct ast_node * line, Common__Request * request) {
    debugp("INSERT COMMAND\n");

}

int parse_delete(struct ast_node * line, Common__Request * request) {
    debugp("DELETE COMMAND\n");
    request->command_code = COMMAND_CODE_DELETE;
    struct ast_node * table = line->first->first->first;
    char * table_name = table->text;
    debugf("Table name: %s\n", table_name);
    request->table_name = table_name;
...
int parse_request_string(const char *in, Common__Request * request) {
    ...
    switch (node_type) {
        ...
        case INSERT:
            return parse_insert(node.first, request);
        case DELETE:
            return parse_delete(node.first, request);
        default:
            return rc;
    }
};
