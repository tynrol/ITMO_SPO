...
void execute_command_insert(Common__Request *request, Common__Response *response) {

}

void execute_command_delete(Common__Request *request, Common__Response *response) {
    response->command_code = request->command_code;
    if (request->n_columns < 1) {
        response->status_code = ERROR_END;
        response->text = ERROR_MESSAGE_NO_COLUMNS;
    }
    int return_code = delete_table(request->table_name);
    debugf("Table deletion finish code: %d\n", return_code);
    response->status_code = return_code;
    define_message_by_return_code(return_code, response, "Success deleted");
}
...
void execute_command(Common__Request *request, Common__Response *response) {
    switch (request->command_code) {
        ...
        case COMMAND_CODE_INSERT:
            execute_command_insert(request, response);
            break;
        case COMMAND_CODE_DELETE:
            execute_command_delete(request, response);
            break;
        default:
